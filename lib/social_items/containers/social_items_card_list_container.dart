...
import 'package:mobagym/social_items/views//social_items_card_list.dart';
...
class SocialItemsCardListContainer extends StatefulWidget {
  ...
  @override
  _SocialItemsCardListContainerState createState() =>
      new _SocialItemsCardListContainerState(_params);
}

class _SocialItemsCardListContainerState
    extends State<SocialItemsCardListContainer> {
    ...
    return StoreConnector<MobagymAppState, _SocialCardList>(
      converter: (store) {
        ...
        vm.refresh = (up) {
          if (up)
            store.dispatch(
                Actions.SocialActions.getItems(_params, forceReload: true));
        };
        var query = store.state.socialState.getQuery(_params);
        if (query == null) {
          Future.delayed(const Duration(milliseconds: 300), () {
            store.dispatch(
                Actions.SocialActions.getItems(_params, forceReload: false));
          });
        } else {
        ...
        if (vm.items.length != 0) loadedItems = vm.items;
        return vm;
      },
  ...
  Widget _body(_SocialCardList viewModel) {
    if (loadedItems.length != 0) {
      ...
      return Text("loading=${viewModel.loading} , error=${viewModel
          .error} , items=${viewModel.items.length}");
  }
}
